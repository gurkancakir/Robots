...
import com.gurkan.robotsgame.genaral.CollisionGeometry;
import com.gurkan.robotsgame.genaral.Config;

import static com.gurkan.robotsgame.genaral.Rectangles.*;

public class Assets {

	private static final String FLYUP_FONT = Config.asString(
			"Global.flyupFont", "ocr_a_small.fnt");
	private static final String SCORE_FONT = Config.asString(
			"Global.scoreFont", "wellbutrin.fnt");
	private static final String TEXT_FONT = Config.asString("Global.textFont",
			"ocr_a.fnt");

	private static final float PLAYER_BORDER_WIDTH = Config.asFloat(
			"Player.borderWidthPercent", 25.0f);
	private static final float PLAYER_BORDER_HEIGHT = Config.asFloat(
			"Player.borderHeightPercent", 6.7f);
	private static final float PLAYER_FRAME_DURATION = Config.asFloat(
			"Player.frameDuration", 0.2f);

	private static final float ROBOT_FRAME_DURATION = Config.asFloat(
			"Robot.frameDuration", 0.1f);

	private static final float CAPTAIN_BORDER_WIDTH = Config.asFloat(
			"Captain.borderWidthPercent", 16.7f);
	private static final float CAPTAIN_BORDER_HEIGHT = Config.asFloat(
			"Captain.borderHeightPercent", 16.7f);
	private static final float CAPTAIN_FRAME_DURATION = Config.asFloat(
			"Captain.frameDuration", 0.1f);

	private static final float PARTICLE_SIZE = Config.asFloat("particle.size",
			0.1875f);
	...
	public static void load() {
		...
		// String textureDir = "data/textures/16";//deneme
		String textureFile = textureDir + "/pack.txt";
		atlas = new TextureAtlas(Gdx.files.internal(textureFile),
				Gdx.files.internal(textureDir));
		loadTextures();
		createAnimations();
		loadFonts();
		loadSounds();
	...
	private static void createAnimations() {
		playerWalkingRightAnimation = new Animation(PLAYER_FRAME_DURATION,
				Assets.playerWalkingRight1, Assets.playerWalkingRight2);
		playerWalkingLeftAnimation = new Animation(PLAYER_FRAME_DURATION,
				Assets.playerWalkingLeft1, Assets.playerWalkingLeft2);

		robotWalkingLeftAnimation = new Animation(ROBOT_FRAME_DURATION,
				robotLeft1, robotLeft2, robotLeft3, robotLeft4, robotLeft3,
				robotLeft2);
		robotWalkingRightAnimation = new Animation(ROBOT_FRAME_DURATION,
				robotRight1, robotRight2, robotRight3, robotRight4,
				robotRight3, robotRight2);
		robotScanningAnimation = new Animation(ROBOT_FRAME_DURATION,
				robotScan1, robotScan2, robotScan3, robotScan4, robotScan3,
				robotScan2);

		nemesisAnimation = new Animation(CAPTAIN_FRAME_DURATION, nemesis1,
				nemesis2);
	}
	...
	private static void loadFonts() {
		...
		scoreFont = new BitmapFont(Gdx.files.internal(fontDir + SCORE_FONT),
				false);
		textFont = new BitmapFont(Gdx.files.internal(fontDir + TEXT_FONT),
				false);
		flyupFont = new BitmapFont(Gdx.files.internal(fontDir + FLYUP_FONT),
				false);
	...
	private static void initialiseGeometries() {
		...
		// TODO: The below is a complete hack just to provide the player and
		// captain with some collision geometry
		// so that he doesn't die when he's clearly not in contact with a wall,
		// bullet or enemy. Ideally it would
