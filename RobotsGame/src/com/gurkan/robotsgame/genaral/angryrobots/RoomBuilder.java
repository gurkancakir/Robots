...
public class RoomBuilder {
	...
	private void createWalls() {
		...
		wallRects = Pools
				.makeArrayFromPool(wallRects, rectanglePool, MAX_WALLS);
	...
	private void createDoors() {
		...
		doorRects = Pools
				.makeArrayFromPool(doorRects, rectanglePool, MAX_DOORS);
		for (int i = 0, j = 0; i < n; i++, j += 4) {
	...
	private float coordPlusHalfHeight(int c, int limit) {
		...
		if (c == 0) {
			...
			return n + WALL_WIDTH - ADJUSTMENT + ((c - 1) * WALL_WIDTH)
					+ HALF_HEIGHT;
		} else {
			return n + 2 * (WALL_WIDTH - ADJUSTMENT) + ((c - 2) * WALL_WIDTH)
					+ HALF_HEIGHT;
		}
	}
