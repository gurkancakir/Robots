...
import com.gurkan.robotsgame.genaral.angryrobots.mobiles.World.FireCommand;
...
public class Robot extends GameObject {
	...
	private static final float WALKING_SPEED = Config.asFloat("Robot.speed",
			1.25f);
	...
	private boolean doesLineHitWall(Rectangle rect, float x1, float y1,
			float x2, float y2) {
		// Does not intersect if minimum y coordinate is below the rectangle.
		float minY = min(y1, y2);
		if (minY >= rect.y + rect.height + fudge)
			return false;

		// Does not intersect if maximum y coordinate is above the rectangle.
		float maxY = max(y1, y2);
		if (maxY < rect.y - fudge)
			return false;

		// Does not intersect if minimum x coordinate is to the right of the
		// rectangle.
		float minX = min(x1, x2);
		if (minX >= rect.x + rect.width + fudge)
			return false;

		// Does not intersect if maximum x coordinate is to the left of the
		// rectangle.
		float maxX = max(x1, x2);
		if (maxX < rect.x - fudge)
			return false;
	...
	private boolean hasLineOfSight(GameObject a, GameObject b) {
		return hasLineOfSight(a.x + a.width / 2, a.y + a.height / 2, b.x
	...
	private boolean hasLineOfSight(float x1, float y1, float x2, float y2) {
		...
		for (int i = 0; i < walls.size; i++) {
			...
			if (Intersector.intersectSegments(robotPos, playerPos, lineStart,
					lineEnd, null)) {
				return false;
			}
		}
