...
import com.gurkan.robotsgame.genaral.Colliders;
import com.gurkan.robotsgame.genaral.Colliders.ColliderHandler;
import com.gurkan.robotsgame.genaral.Colliders.RemovalHandler;
import com.gurkan.robotsgame.genaral.Colliders.SceneryHandler;
import com.gurkan.robotsgame.genaral.Config;
import com.gurkan.robotsgame.genaral.Grid;
import com.gurkan.robotsgame.genaral.Pools;
import com.gurkan.robotsgame.genaral.angryrobots.DifficultyManager;
import com.gurkan.robotsgame.genaral.angryrobots.DoorPositions;
import com.gurkan.robotsgame.genaral.angryrobots.RoomBuilder;
import com.gurkan.robotsgame.genaral.angryrobots.WorldListener;
import com.gurkan.robotsgame.genaral.angryrobots.WorldNotifier;
import com.gurkan.robotsgame.genaral.angryrobots.mobiles.BaseShot;
import com.gurkan.robotsgame.genaral.angryrobots.mobiles.Captain;
import com.gurkan.robotsgame.genaral.angryrobots.mobiles.GameObject;
import com.gurkan.robotsgame.genaral.angryrobots.mobiles.Player;
import com.gurkan.robotsgame.genaral.angryrobots.mobiles.PlayerShot;
import com.gurkan.robotsgame.genaral.angryrobots.mobiles.Robot;
import com.gurkan.robotsgame.genaral.angryrobots.mobiles.RobotShot;

import static com.badlogic.gdx.math.MathUtils.*;
import static com.gurkan.robotsgame.genaral.MathUtils.*;

/**
 * The <code>World</code> is the representation of the game world of <b>Very
 * Angry Robots</b>. It knows nothing about how it will be displayed, neither
 * does it know about how the player is controlled, particle effects, sounds,
 * nor anything else. It purely knows about the {@link Player}, {@link Robot}s
 * and the walls of the room that the player is in.
 * 
 * @author Rod
 */
public class World {
	...
	/**
	 * The <code>FireCommand</code> interface is how the {@link World} is told
	 * that a {@link GameObject} wants to fire.
	 */
	public static interface FireCommand {
		/**
		 * Tells the {@link World} that a {@link GameObject} wants to fire. Note
		 * that <code>dx</code> and <code>dy</code> must be normalised. The
		 * World does not have to fire just because it is asked to.
		 * 
		 * @param firer
		 *            the {@link GameObject} that wants to fire.
		 * @param dx
		 *            the horizontal component of the bullet's direction.
		 * @param dy
		 *            the vertical component of the bullet's direction.
		 */
	...
	private static final int MAX_PLAYER_SHOTS = Config.asInt("Player.maxShots",
			4);
	private static final int MAX_ROBOT_SHOTS = Config
			.asInt("Robot.maxShots", 6);
	private static final int MAX_ROBOTS = Config.asInt("Global.maxRobots", 12);
	private static final float PLAYER_DEAD_INTERVAL = Config.asFloat(
			"Global.deadTime", 2.0f);
	private static final float FIRING_AMNESTY_INTERVAL = Config.asFloat(
			"Global.amnestyTime", 2.0f);
	private static final float CAPTAIN_LURK_MULTIPLIER = Config.asFloat(
			"Captain.lurkMultiplier", 2.0f);
	private static final float CAPTAIN_MIN_DELAY = Config.asFloat(
			"Captain.minLurkTime", 2.0f);
	static final float FIRING_INTERVAL = Config.asFloat(
			"Player.firingInterval", 0.25f);
	public static final float ROOM_TRANSITION_TIME = Config.asFloat(
			"Global.roomTransitionTime", 0.5f);
	...
	void addRobotShot(GameObject firer, float dx, float dy) {
		if (state == PLAYING && robotShots.size < numRobotShots
				&& stateTime >= FIRING_AMNESTY_INTERVAL) {
	...
	private void placeCaptain() {
		...
		captain.activateAfter(max(CAPTAIN_MIN_DELAY, CAPTAIN_LURK_MULTIPLIER
				* robots.size));
	...
	private boolean canSpawnHere(Robot robot) {
		return !(intersectsWalls(robot) || intersectsDoors(robot)
				|| intersectsRobots(robot) || playerPos.dst(robot.x, robot.y) < WALL_WIDTH);
	}
	...
	private void createPlayerShots() {
		playerShots = Pools.makeArrayFromPool(playerShots, shotPool,
				MAX_PLAYER_SHOTS);
	}

	private void createRobotShots() {
		robotShots = Pools.makeArrayFromPool(robotShots, robotShotPool,
				MAX_ROBOT_SHOTS);
	}
	...
	private void checkMobileMobileCollisions() {
		...
		Colliders.collide(captain, playerShots,
				captainGameObjectCollisionHandler);
		Colliders.collide(captain, robotShots,
				captainGameObjectCollisionHandler);
	}

	private void checkMobileSceneryCollisions() {
		Colliders.collide(player, roomGrid.get(player.bounds()),
				playerSceneryHandler);
	...
	private <U extends GameObject, T extends U> void markSceneryCollisions(
			Array<T> gos, SceneryHandler<U> handler) {
	...
	private void removeMarkedMobiles() {
		...
		Colliders.removeMarkedCollisions(shotPool, playerShots,
				shotRemovalHandler);
		Colliders
				.removeMarkedCollisions(robotPool, robots, robotRemovalHandler);
		Colliders.removeMarkedCollisions(robotShotPool, robotShots,
				shotRemovalHandler);
	}
