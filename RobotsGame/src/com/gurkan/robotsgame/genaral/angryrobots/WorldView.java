...
import com.gurkan.robotsgame.genaral.Particle;
...
/**
 * The <code>WorldView</code> displays the {@link World} on screen. It also
 * provides the means by which the player can control the game.
 * 
 * @author Rod
 */
public class WorldView {
	...
	/**
	 * The <code>Presenter</code> interface is how the <code>WorldView</code>
	 * communicates the state of its controls.
	 */
	...
	private static final float FIRING_DEAD_ZONE = 0.125f; // TODO: add to
															// Config.
	private static final float JOYSTICK_DISTANCE_MULTIPLIER = 0.2f; // TODO: add
																	// to
																	// Config.
	 ...
	 * 
	 * @param world
	 *            the {@link World} that this is a view of.
	 * @param presenter
	 *            the interface by which this <code>WorldView</code>
	 *            communicates the state of its controls.
	 */
	...
	public void render(float delta) {
		...
		case World.PLAYING:
			// TODO: this is really a bit of a hack ... does it really need to
	...
	private int createWallsAndDoors(SpriteCache sc) {
		...
		for (int i = 0; i < rects.size; i++) {
			...
			sc.add(Assets.pureWhiteTextureRegion, rect.x, rect.y, rect.width,
					rect.height);
		}
		sc.setColor(1, 1, 0, 1);
	...
	private void updatePanning() {
		// If we're moving from one room to another then we want to draw the old
		// room scrolling off as the new room
		// scrolls on.
		final float w = worldWidth;
		final float h = worldHeight;
		float time = min(1.0f, world.getStateTime()
				/ World.ROOM_TRANSITION_TIME);
	...
	private void drawCaptain() {
		...
		if (captain.state == Captain.CHASING) {
			drawClipped(captain, Assets.nemesisAnimation.getKeyFrame(
					captain.stateTime, true));
		}
	}
	...
	private void drawPlayer() {
		...
		case Player.WALKING_LEFT:
			draw(player, Assets.playerWalkingLeftAnimation.getKeyFrame(
					player.stateTime, true));
			break;
		case Player.WALKING_RIGHT:
			draw(player, Assets.playerWalkingRightAnimation.getKeyFrame(
					player.stateTime, true));
			break;
	...
	private void drawClipped(GameObject go, TextureRegion region) {
		float boundsMinX = worldMinX + World.OUTER_WALL_ADJUST
				+ World.WALL_HEIGHT;
		float boundsMaxX = worldMaxX - World.OUTER_WALL_ADJUST
				- World.WALL_HEIGHT;
		float boundsMinY = worldMinY + World.OUTER_WALL_ADJUST
				+ World.WALL_HEIGHT;
		float boundsMaxY = worldMaxY - World.OUTER_WALL_ADJUST
				- World.WALL_HEIGHT;
		...
		spriteBatch.draw(region.getTexture(), x, y, width, height, srcX, srcY,
	...
	private void drawParticles() {
		...
		for (Particle particle : particleManager.getParticles()) {
			if (particle.active) {
				...
				spriteBatch.draw(Assets.pureWhiteTextureRegion, particle.x,
						particle.y, particle.size, particle.size);
			}
		}
	...
	public void updateControls(float delta) {
		...
		if (Gdx.input.justTouched()) {
			worldCam.unproject(touchPoint.set(Gdx.input.getX(),
					Gdx.input.getY(), 0));
			if (world.isPaused()) {
				presenter.resume();
			...
			worldCam.unproject(dragPoint.set(Gdx.input.getX(),
					Gdx.input.getY(), 0));
