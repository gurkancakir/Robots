...
public class StatusManager implements WorldListener {
	...
	public enum Achievements {
		PERFECTIONIST("Perfectionist", "Clear " + CLEAN_ROOMS
				+ " rooms in a row."), PERFECT_SHOT("Perfect Shot",
				"Clear a room by shooting everything."), PACIFICIST("Pacifist",
				"Clear a room without shooting anything."), DAREDEVIL(
				"Daredevil", "Survive for " + DAREDEVIL_SECONDS
						+ " seconds after the captain enters the room."), COWARD(
				"Coward", "Run from " + CHICKEN_ROOMS + " rooms in a row."), ADDICT(
				"Addict", "Play " + ADDICT_GAMES + " games."), LUCKY_JIM(
				"Lucky Jim", "Survive for " + LUCKY_JIM_ROOMS
						+ " rooms without being hit."), ROBOCIDE("Robocide",
				"Destroyed " + ROBOCIDE_ROBOTS + " robots");
	...
	// Achievements.
	static final int CLEAN_ROOMS = Config.asInt(
			"achievements.cleanRoomsForPerfectionist", 5);
	static final int ROBOCIDE_ROBOTS = Config.asInt(
			"achievements.robotsForRobocide", 50);
	static final int LUCKY_JIM_ROOMS = Config.asInt(
			"achievements.roomsForLuckyJim", 10);
	static final int CHICKEN_ROOMS = Config.asInt(
			"achievements.roomsForChicken", 5);
	static final int ADDICT_GAMES = Config.asInt("achievements.gamesForAddict",
			5);
	static final float DAREDEVIL_SECONDS = Config.asInt(
			"achievements.daredevilSeconds", 10);
	...
	private static final int ROBOT_BONUS_SCORE = Config.asInt(
			"Robot.bonusScore", 100);
	private static final int EXTRA_LIFE_SCORE_1 = Config.asInt(
			"Player.firstExtraLife", 10000);
	private static final int EXTRA_LIFE_SCORE_2 = Config.asInt(
			"Player.secondExtraLife", 50000);
	...
	/**
	 * The number of robots that have been destroyed in this room (including
	...
	/**
	 * The number of robots that have been destroyed in this game (including
	 * those that have been shot).
	 */
	...
	public void onRobotDestroyed(Robot robot) {
		...
		addScore(robotScore);
		scoringEventNotifier.onScoringEvent(robot.x + robot.width / 2, robot.y
				+ robot.height / 2, robotScore);
